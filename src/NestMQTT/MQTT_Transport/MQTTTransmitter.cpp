...
bool Transmitter::sendConnectionRequest() {
  ...
  if (_client->getClientState() == StateMachine::State::disconnected) {
    MQTT_SEMAPHORE_TAKE();
    if (_addPacketFront(_clientCfg.connections_settings._cleanSession,
                        ...
                        (uint16_t)(_clientCfg.connections_settings._keepAlive /
                                   1000), // 32b to 16b doesn't overflow because
                                          // it comes from 16b originally
                        _clientCfg.set_null_client_id ? nullptr
                                                      : _clientCfg.path)) {
      result = true;
